...
public class MealRequestService {
    ...
    public MealRequest createRequest(
        ...
        if (user.role() != UserRole.MEMBER && user.role() != UserRole.MANAGER) {
        ...
        if (attendeeCount <= 0) {
            throw new IllegalArgumentException("참석 인원 수는 1명 이상이어야 합니다.");
        }
        if (reason == null || reason.isBlank()) {
            throw new IllegalArgumentException("회식 사유는 필수입니다.");
        }
        if ((lateNightPlanned || alcoholPlanned) && (specialReason == null || specialReason.isBlank())) {
            throw new IllegalArgumentException("심야 또는 주류 예정 시 사유가 필요합니다.");
        }

        int expectedAmount = attendeeCount * mealType.perPersonLimit();
    ...
    public MealRequest approveByManager(String managerUserId, String requestId) {
        UserAccount manager = requireUser(managerUserId);
        if (manager.role() != UserRole.MANAGER) {
            throw new IllegalStateException("MANAGER만 승인할 수 있습니다.");
        }
        ...
        if (!request.teamId().equals(manager.teamId())) {
            throw new IllegalStateException("다른 팀의 신청은 승인할 수 없습니다.");
        }
        ...
        String approvalNumber = "AP-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
    ...
    public MealRequest rejectByManager(String managerUserId, String requestId) {
        UserAccount manager = requireUser(managerUserId);
        if (manager.role() != UserRole.MANAGER) {
            throw new IllegalStateException("MANAGER만 반려할 수 있습니다.");
        }
        ...
        if (!request.teamId().equals(manager.teamId())) {
            throw new IllegalStateException("다른 팀의 신청은 반려할 수 없습니다.");
        }

        MealRequest rejected = request.withStatus(RequestStatus.REJECTED, null, null);
